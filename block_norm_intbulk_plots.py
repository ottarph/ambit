...
results_base = "demos/fsi/tmp_tri/1proc/results_fsi_channel_flag_turek_FSI2_r3000_"
results_post = ".txt"
...
block_norms_arr = np.loadtxt(results_base+"newton_iter_blocknormsALEint.txt", skiprows=1, delimiter=',')[:,1:]
...
block_norms = [[None for _ in range(6)] for __ in range(6)]
...
for e, (i,j) in enumerate(blocks):
    ...
    print(f"{i,j = }: {np.std(block_norms_arr[:,e]) = :.2e}, {np.mean(block_norms_arr[:,e]) = :.2e}")
    # print(f"{i,j = }: {A[i,j] = }")
...
plt.colorbar(im)
plt.savefig("mean_blocknorm_intbulk_logscale.pdf")
